...
$("saveCompetition").addEventListener("click",()=>{
  const count=+$("compTeamCount").value, teams=state.teams.slice(0,count).map(t=>t.id), comp={id:uid(),name:$("compName").value.trim()||"Custom Championship",structure:$("compStructure").value,format:$("compFormat").value,teams,rr:+$("compRR").value,hasFinals:$("compFinals").value==="Yes",finalTeams:+$("compFinalTeams").value,winPts:+$("compWinPts").value,tiePts:+$("compTiePts").value,superOver:$("compSuperOver").value==="Enabled",decisionTimer:+$("decisionTimer").value,createdAt:nowISO(),managerSlots:{},fixtures:[]};
...
async function createOnlineRoom(){
  ...
  const r=ensureLocalRoom();let c=r.tournamentId?C(r.tournamentId):null;try{
...
$("hostStartMatch").addEventListener("click",async()=>{
  ...
  save();if(r.cloudId&&session)await api("/rest/v1/matches?id=eq."+r.cloudId,{method:"PATCH",body:JSON.stringify({status:"live",started_at:nowISO()})}).catch(()=>{});prepareMatch(r);nav("toss");setTimeout(()=>{$("tossResult").textContent=(Math.random()<.5?T(r.teamA).name:T(r.teamB).name)+" won the toss and elected to bowl."},900)
});
$("tossContinue").addEventListener("click",()=>{nav("liveMatch");showPlayerIntro(match.bowlingTeam);updateHUD();addCom(`${match.battingTeam.name} begin the innings.`,"system")});
...
async function requestDecision(type,team,options){
  const managerName=managerForTeam(team.id),timeout=match.room.decisionTimer||state.settings.timeout;
  if(!state.settings.managerControl||!managerName)return type==="next_bowler"?aiBowler():options[0];
...
function aggression(){const p=match.striker,ov=match.balls/6,left=120-match.balls;let desire=.38+ov*.022;if(match.target){const need=Math.max(0,match.target-match.score),rr=need/Math.max(1,left/6);desire=.29+(rr-6)/13+(ov/20)*.13;if(left<=30)desire+=.07;if(left<=12)desire+=.08;if(match.wickets>=7)desire-=.10}return clamp(.45*(p.aggression/100)+.55*clamp(desire,.18,.98),.18,.98)}
function outcome(){const p=match.striker,b=match.currentBowler,a=aggression();let wp=clamp(.022+(b.bowlingSkill-70)*.0006+a*.02-(p.batting-70)*.0004,.011,.09);if(Math.random()<wp)return{wicket:true,runs:0};const q=clamp(.52+(p.batting-b.bowlingSkill)*.003+(p.power-70)*.0017+a*.085,.25,.87),p6=clamp(.018+a*.05+(p.power-70)*.0012,.008,.13),p4=clamp(.065+a*.072+(p.batting-70)*.0009,.04,.18),p2=.105+q*.035,p0=clamp(.27-q*.10-a*.04,.08,.30),x=Math.random();let runs=1;if(x<p6)runs=6;else if(x<p6+p4)runs=4;else if(x<p6+p4+p2)runs=2;else if(x<p6+p4+p2+.008)runs=3;else if(x<p6+p4+p2+.008+p0)runs=0;return thriller({wicket:false,runs})}
...
function ballDuration(){return $("paceSelect").value==="real"?15000:1400}
...
function updateHUD(){if(!match)return;$("hudScore").textContent=`${match.score}-${match.wickets}`;$("hudOvers").textContent=`${Math.floor(match.balls/6)}.${match.balls%6} OVERS`;$("hudRate").textContent=`RUN RATE ${match.balls?(match.score/(match.balls/6)).toFixed(2):"0.00"}`;$("hudStriker").textContent=(match.striker?.name||"—").toUpperCase();$("hudNon").textContent=(match.non?.name||"—").toUpperCase();$("hudStrikerRuns").textContent=`${match.batterRuns[match.striker?.id]||0} ${match.batterBalls[match.striker?.id]||0}`;$("hudBowler").textContent=(match.currentBowler?.name||"—").toUpperCase();const bid=match.currentBowler?.id;$("hudBowlingFigures").textContent=bid?`${match.bowlerRuns[bid]||0}-${0} (${((match.bowlerBalls[bid]||0)/6).toFixed(1)})`:"";$("lastBalls").innerHTML=match.lastBalls.slice(-6).map(x=>`<i>${x}</i>`).join("");$("activeRoleDisplay").textContent=state.localRole.toUpperCase();$("footerRole").textContent="ROLE: "+state.localRole.toUpperCase()}
async function playBall(){if(!match||match.completed||ballLock||paused||state.localRole==="spectator")return;ballLock=true;const started=Date.now();
  if(!match.currentBowler){match.currentBowler=await requestDecision("next_bowler",match.bowlingTeam,availableBowlers());match.pendingDecision=null;addCom(`${match.currentBowler.name} will bowl over ${Math.floor(match.balls/6)+1}.`,"system")}
  updateHUD();const o=outcome(),no=`${Math.floor(match.balls/6)}.${match.balls%6+1}`,striker=match.striker,bowler=match.currentBowler;match.batterBalls[striker.id]=(match.batterBalls[striker.id]||0)+1;stat(striker.id).balls++;
  await animateDelivery(o);match.balls++;match.bowlerBalls[bowler.id]=(match.bowlerBalls[bowler.id]||0)+1;
  if(o.wicket){match.wickets++;stat(striker.id).outs++;stat(bowler.id).wickets++;match.lastBalls.push("W");addCom(`${no} ${bowler.name} to ${striker.name} — WICKET! The pressure brings a breakthrough.`,"wicket");splash("WICKET!");if(!inningsDone()){const opts=match.order.slice(match.nextIndex);match.striker=await requestDecision("next_batter",match.battingTeam,opts);match.nextIndex=Math.max(match.nextIndex,match.order.indexOf(match.striker)+1);match.pendingDecision=null;addCom(`${match.striker.name} is the new batter.`,"system")}}
  else{match.score+=o.runs;match.batterRuns[striker.id]=(match.batterRuns[striker.id]||0)+o.runs;match.bowlerRuns[bowler.id]=(match.bowlerRuns[bowler.id]||0)+o.runs;stat(striker.id).runs+=o.runs;stat(bowler.id).conceded+=o.runs;if(o.runs===4)stat(striker.id).fours++;if(o.runs===6)stat(striker.id).sixes++;match.lastBalls.push(String(o.runs));const cls=o.runs===6?"six":o.runs===4?"four":"";if(o.runs===6){addCom(`${no} ${bowler.name} to ${striker.name} — SIX! Launched over the rope.`,"six");splash("SIX!")}else if(o.runs===4){addCom(`${no} ${bowler.name} to ${striker.name} — FOUR! Timed perfectly into the gap.`,"four");splash("FOUR!")}else if(o.runs===0)addCom(`${no} ${bowler.name} to ${striker.name} — Dot ball.`);else addCom(`${no} ${bowler.name} to ${striker.name} — ${o.runs} run${o.runs===1?"":"s"}.`);if(o.runs%2){const t=match.striker;match.striker=match.non;match.non=t}}
  ...
  updateHUD();await publishBallEvent(o,no);await publishMatchState();if(inningsDone())await endInnings();const rest=ballDuration()-(Date.now()-started);if(rest>0)await sleep(rest);ballLock=false}
...
async function completeCloudMatch(result,so){if(match.room.cloudId&&session)await api("/rest/v1/matches?id=eq."+match.room.cloudId,{method:"PATCH",body:JSON.stringify({status:"completed",result:{result,superOver:so,first:match.first,second:{score:match.score,wickets:match.wickets}},state:serializeMatch(),completed_at:nowISO()})}).catch(()=>{})}
function serializeMatch(){if(!match)return{};return{innings:match.innings,score:match.score,wickets:match.wickets,balls:match.balls,target:match.target,battingTeam:match.battingTeam.id,bowlingTeam:match.bowlingTeam.id,striker:match.striker?.id,non:match.non?.id,bowler:match.currentBowler?.id,lastBalls:match.lastBalls,pendingDecision:match.pendingDecision,completed:match.completed}}
async function publishMatchState(){if(match?.room?.cloudId&&session&&state.localRole==="host")await api("/rest/v1/matches?id=eq."+match.room.cloudId,{method:"PATCH",body:JSON.stringify({state:serializeMatch(),updated_at:nowISO()})}).catch(()=>{})}
...
$("oneBallBtn").addEventListener("click",playBall);$("pauseBtn").addEventListener("click",()=>{paused=true;nav("pauseMenu")});qsa('[data-nav="liveMatch"]').forEach(b=>b.addEventListener("click",()=>{paused=false}));
...
async function watchByCode(c){if(!c)return;if(cloudReady()){try{const rows=await api("/rest/v1/matches?share_code=eq."+encodeURIComponent(c)+"&select=*");if(!rows.length)return alert("Match not found.");const m=rows[0];state.localRole="spectator";state.localRoom={id:m.id,cloudId:m.id,cloudTournamentId:m.tournament_id,teamA:m.team_a_local_id,teamB:m.team_b_local_id,shareCode:m.share_code,status:m.status,visibility:m.visibility,decisionTimer:m.decision_timeout};save();nav("liveMatch");startSpectatorPolling()}catch(e){alert(e.message)}}else{const r=ensureLocalRoom();if(c!==r.shareCode)return alert("Local demo match code not found.");state.localRole="spectator";save();nav("liveMatch")}}
